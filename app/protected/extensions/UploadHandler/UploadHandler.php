...
class UploadHandler
{
    ...
    function __construct($options = null)
    {
        $this->options = array(
            ...
            'image_versions' => array(
                ...
                */
                'thumbnail' => array(
                    'upload_dir' => Yii::app()->params['thumb_dir'],
                    'upload_url' => Yii::app()->params['thumb_url'],
                    'max_width' => 80,
                    'max_height' => 80
                )
            )
    ...
    private function getFilePath($fileName, $thumb = false)
    {
        ...
        $path = $root . date("Y")."/";
        if (!is_dir($path . date("m"))) mkdir($path . date("m"), 0777);
        $path = $path . date("m")."/";
        if (!is_dir($path . date("d"))) mkdir($path . date("d"), 0777);
        $path = $path . date("d");
        $md5 = md5($fileName);
